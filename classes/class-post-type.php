...
class MPT_Post_Type {
	...
	public function __construct() {
		...
		// Member pending
		add_action( 'transition_post_status', array( __CLASS__, 'set_validation_registration_key' ), 10, 3 );
	}
	...
	/**
	 * Set validation registration key
	 *
	 ...
	 * @return void
	 */
	public static function set_validation_registration_key( string $new_status, string $old_status, \WP_Post $current_post ) {
		if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || \wp_is_post_revision( $current_post ) ) {
			return;
		}
		...
		$member = new MPT_Member( $current_post );

		if ( ! $member->exists() ) {
			return;
		}

		$member->generate_validation_registration_key( $member->get_member_key() );
	}
}

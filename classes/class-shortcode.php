...
class MPT_Shortcode {
	...
	public static function get_messages( $format = 'display' ) {
		...
		if ( $format == 'display' && !empty($mpt_messages) ) {
			$output = '<div class="mpt-messages">';
			foreach( $mpt_messages as $field => $message ) {
				$output .= '<div class="mpt-field notice-' . esc_attr($message['status']) . '">';
				$output .= '<p class="field-target-' . esc_attr($field) . ' ' . esc_attr($message['status']) . '">' . stripslashes($message['message']) . '</p>';
				$output .= '</div>';
			}
			$output .= '</div>';
			return $output;
